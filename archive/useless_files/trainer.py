...
class PPOTrainer:
    ...
    def compute_advantages(self, rewards, values, dones):
        ...
        for t in reversed(range(len(rewards))):
            ...
            # TD = reward + gamma * next_value - current_value
            td_error = rewards[t] + self.gamma * next_value - values[t]
            # Calculate advantage using GAE formula
            # advantage = TD + gamma * lambda * previous_advantage
            advantage = td_error + self.gamma * self.gae_lambda * advantage
            # Store advantage for this step
            advantages.insert(0, advantage)  # Insert at beginning (we're going backwards)
            # Calculate retur: advantage + value
